...
容器默认以 `1000:1000` 身份运行。**如果你的宿主机用户不是 1000，写入会失败**，
表现为同步报权限错误。查询并修改：

```bash
id -u    # 例如输出 1026，就设 PUID=1026
id -g    # 例如输出 100，就设 PGID=100
```
...
| 平台                                       | 是否需要改                                    |
| ------------------------------------------ | --------------------------------------------- |
| **Linux / NAS（群晖、威联通、Unraid 等）** | **需要确认**。请务必用 `id -u` / `id -g` 核对 |
| macOS / Windows 上的 Docker Desktop        | 一般不用改，桌面版的挂载不强制 UID            |

> **注意**：`PUID` / `PGID` 同时影响**管理端容器**和**命令行容器**——因为两者都会写入
> `data/`（管理端的上传、删除、同步都需要写权限）。它不是命令行专属配置。
...
`PUID` / `PGID` 与宿主机文件属主不一致，多见于 Linux / NAS。用 `id -u` 与 `id -g`
查出正确值并写进 `.env`，然后重启。
