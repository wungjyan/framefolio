...
| 类型 | 最长边 | WebP 质量 | 主要用途 |
| --- | ---: | ---: | --- |
| Thumbnail | 960 px | 82 | 移动单列、Justified 和较小的 Editorial 图片 |
| Preview | 2560 px | 88 | Editorial 大图和 Viewer |
...
- [ ] 建立全局中性色、字体、间距、容器宽度和响应式断点变量。
- [ ] 完成首页数据获取、加载、空数据和错误状态。
- [ ] 实现共用 `GalleryImage`，统一处理宽高、懒加载、图片选择和打开 Viewer。
- [ ] 为首屏重点照片设置更高加载优先级，其余照片延迟加载。
- [ ] 预留桌面布局切换区域，移动端隐藏该区域。
...
- [ ] 将照片按宽高比分为横向、竖向和近方形。
- [ ] 定义 4–5 组确定性 CSS Grid 排版 pattern。
- [ ] 根据照片方向选择可用 pattern，稳定循环而不使用随机布局。
- [ ] 在大图、小图、左右位置和垂直错位之间建立节奏。
- [ ] 在各种照片数量和横竖比组合下提供安全降级布局。

#### 4.2 Justified

- [ ] 根据照片宽高比累加组行。
- [ ] 为每行计算共享高度，使非末行填满容器。
- [ ] 末行保持目标高度和自然宽度，不强行拉伸填满。
- [ ] 使用 `ResizeObserver` 在容器尺寸变化后重新计算。
- [ ] 保证所有照片完整显示且不裁剪。
...
- [ ] 默认使用 `editorial`。
- [ ] 仅在桌面端显示两个极简图标控件。
- [ ] 控件具有可识别的焦点态、可访问名称和简短 Tooltip。
- [ ] 在本地保存用户最后一次选择；储存不可用时回退到 Editorial。

**验收标准**

- Editorial 不呈现传统瀑布流效果，刷新后排版保持一致。
...
- [ ] 使用语义化 dialog 实现全屏 Viewer。
- [ ] 支持点击照片打开、关闭控件和 `Escape` 关闭。
- [ ] 支持上一张、下一张控件及左右方向键。
- [ ] 打开时锁定背景滚动，关闭后恢复原状。
- [ ] 实现焦点约束，关闭后将焦点返回触发照片。
- [ ] 预加载相邻 preview 图片。
- [ ] 按实际存在的字段组合相机、镜头、参数和日期。
- [ ] 信息层级弱于照片，不使用独立卡片或装饰容器。
...
- [ ] 检查 Editorial 的留白、横竖图节奏和 pattern 循环稳定性。
...
- [x] 同步失败不会损坏上一份可用索引。
- [ ] 网站运行时只读取 `photos.json` 和 `generated`。
- [ ] 原图和 GPS 元数据没有公开访问路径。
- [ ] 桌面端可在 Editorial 和 Justified 之间切换。
- [ ] 移动端只使用单列照片流。
- [ ] Viewer 支持关闭、前后导航、键盘操作和缺失字段友好的 EXIF 展示。
- [ ] 图片在 Gallery 和 Viewer 中均不被裁剪。
