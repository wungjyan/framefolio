...
> **历史文档，保留原貌。** 本文记录最初 MVP 的实施计划，其中部分内容已被后续重构取代：
>
> - 索引结构已升级到 **schema v2**（存储键替代烘焙的 URL），见 [`REFACTOR_PLAN.md`](./REFACTOR_PLAN.md)
> - 同步除 CLI 外，还可由 `/admin` 管理端的「立即同步」按钮触发（仍共用同一把锁）
> - `data` 挂载由只读改为可写，因为管理端需要上传与软删除
> - 「默认使用 `editorial`」与「在本地保存用户最后一次选择」两处勾选有误，已就地更正（见 §4.4）
>
> 需要当前行为说明时，请以 `DEV.md`、`README.md` 与 `REFACTOR_PLAN.md` 为准。
...
- [x] 默认使用 `justified`（与 `app/composables/useGalleryLayout.ts` 一致；此处原记录为 `editorial`，属文档漂移，已修正）。
...
- [ ] **未实现**：在本地保存用户最后一次选择（布局选择为会话内状态，`layout` 是裸 `ref`，无任何存储读写）。原记录误标为已完成，现已更正；该能力已移出本次重构范围。
